...
06|7| 83%|監視運用とログ設計（講義No.32）  <-- いまここ
...
06|06-5|8|100%|主要業務イベントのINFOログ（登録・投稿作成/削除）      PR #98
06|06-6|0|  0%|機密情報チェック＋運用設計書（docs/operations.md）  <-- いまここ
"
...
# ── 次にやること（番号|内容） ────────────────────────────────
NEXT="
1|Issue6: 機密情報チェック＋運用設計書（docs/operations.md 新規作成）
2|デプロイ（EC2 + RDS + ALB）  <-- あなたの明示指示があるまで着手しない
"

# フェーズ06（監視運用とログ設計、講義No.32）が進行中。6 Issue中5つ完了（#89,#91,#93,#95,#97）。
