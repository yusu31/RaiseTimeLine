...
04|8|100%|テストの整備
05|3| 33%|品質の詰め（バグ修正・想定外テスト）  <-- いまここ
06|0|  - |デプロイ（AWS）
"
...
05|05-2|0|  0%|想定外のテスト（極端な入力・同時操作）
05|05-3|0|  0%|ブランチ保護の必須チェック設定（あなたのGitHub操作）
"
...
# ── 次にやること（番号|内容） ────────────────────────────────
NEXT="
1|いいね失敗時にエラーが出ない件（testing-design.md 11-7）   <-- PR #76 のテストが見つけた
