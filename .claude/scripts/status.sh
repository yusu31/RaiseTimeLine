...
04|8|100%|テストの整備
05|7| 83%|品質の詰め（バグ修正・想定外テスト）  <-- いまここ（残りはブランチ保護だけ）
06|0|  - |デプロイ（AWS）
"
...
# 探索で見つけた3件（A・B・C）はこれで全部直した。残りは 05-4 だけ。
...
# ── 次にやること（番号|内容） ────────────────────────────────
NEXT="
1|エラー表示が成功しても消えない件（testing-design.md 11-6）  <-- 4画面まとめて直す
2|ブランチ保護の必須チェック設定（あなたのGitHub操作。これでフェーズ05が完了）
3|デプロイ（あなたの指示待ち）
"

# 1 は「同じ問題が複数箇所にある」型。1か所だけ直すと画面ごとに挙動が食い違うので、
# 4画面＋他の操作をまとめて直す（PR #78 で決めた方針）。
# 2 は GitHub の設定画面での作業。コードは1行も変わらないが、あなたの操作が要る。
