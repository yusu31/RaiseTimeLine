...
04|8|100%|テストの整備
05|7| 86%|品質の詰め（バグ修正・想定外テスト）  <-- いまここ（残りはブランチ保護だけ）
06|0|  - |デプロイ（AWS）
"
...
# ── 次にやること（番号|内容） ────────────────────────────────
NEXT="
1|ブランチ保護の必須チェック設定（あなたのGitHub操作。これでフェーズ05が完了）
2|デプロイ（あなたの指示待ち）
"

# 1 は GitHub の設定画面での作業。コードは1行も変わらないが、あなたの操作が要る。
# これが終わるとフェーズ05は100%になり、残るテーマはデプロイだけになる。
