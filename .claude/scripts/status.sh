...
04|8|100%|テストの整備
05|4| 50%|品質の詰め（バグ修正・想定外テスト）  <-- いまここ（探索で工程が2つ増えた）
06|0|  - |デプロイ（AWS）
"
...
05|05-5|0|  0%|【新】例外ハンドラの取りこぼしで500になる件を直す（A・C）
05|05-6|0|  0%|【新】ページ番号のけた溢れで500になる件を直す（B・4か所）
"

# 05-5・05-6 は 05-3（PR #80）の探索で見つかった。
# 「探しに行く回」なので工程が増えるのは想定内。分母が増えた結果、
# 完了は 2/4（50%）から 3/6（50%）になった。数字は同じだが中身は進んでいる。
...
# ── 次にやること（番号|内容） ────────────────────────────────
NEXT="
1|例外ハンドラの取りこぼしで500になる件（testing-design.md 14-3 / 14-4）  <-- A と C をまとめて
2|ページ番号のけた溢れで500になる件（testing-design.md 14-2）  <-- PostService 4か所まとめて
