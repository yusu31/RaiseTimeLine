...
04|8|100%|テストの整備
05|5| 67%|品質の詰め（バグ修正・想定外テスト）  <-- いまここ（探索で増えた2工程のうち1つ完了）
06|0|  - |デプロイ（AWS）
"
...
# ── 次にやること（番号|内容） ────────────────────────────────
NEXT="
1|ページ番号のけた溢れで500になる件（testing-design.md 14-2）  <-- PostService 4か所まとめて
2|エラー表示が成功しても消えない件（testing-design.md 11-6）  <-- 4画面まとめて直す
3|ブランチ保護の必須チェック設定
4|デプロイ（あなたの指示待ち）
"

# 1〜3 はいずれも「同じ問題が複数箇所にある」型。1か所だけ直すと画面ごと・APIごとに
# 挙動が食い違うので、必ずまとめて直す（PR #78 で決めた方針）。
