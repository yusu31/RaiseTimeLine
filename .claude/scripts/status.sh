...
06|0|  0%|デプロイ（AWS: EC2 + RDS + ALB）  <-- いまここ（あなたの指示待ち）
"
...
2|残っている将来課題（testing-design.md / features/*.md に記録済み。急ぎではない）
"

# フェーズ01〜05まで完了。残るテーマはデプロイだけ。
# デプロイは課金が発生するため、あなたの明示指示があるまで着手しない（2026-09-03に明言）。
