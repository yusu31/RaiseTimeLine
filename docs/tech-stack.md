...
**CIが緑でないと `main` にマージできない。** GitHub の Ruleset `protect-main` で強制している。

| ルール | 内容 |
|---|---|
| `pull_request` | PRを通さないと `main` に入れられない（承認は0人でよい） |
| `non_fast_forward` | force push 禁止 |
| `deletion` | `main` ブランチの削除禁止 |
| **`required_status_checks`** | **下記2つが成功するまでマージできない** |

必須にしているチェックは、`ci.yml` の**ジョブ名とまったく同じ文字列**で登録する。

- `フロントエンド（lint・テスト・ビルド）`
- `バックエンド（ビルド・テスト）`

> 🚨 **`ci.yml` の `name:` を変えたら、必須チェックの登録名も一緒に変える。**
> 名前が一致しなくなると、**そのチェックは永遠に完了しない**（待ち続けてマージできない）か、
> **必須から外れて素通りになる**。どちらも気づきにくいので、ジョブ名の変更はセットで扱う。

#### 入れなかった設定と理由

| 設定 | 判断 | 理由 |
|---|---|---|
| Require branches to be up to date | **入れない** | `main` が進むたび全PRで取り込みが必要になる。**一人開発でPRを1本ずつ出す進め方では起きない問題**に毎回手間を払うことになる。規模が大きくなったらマージキューで対応する |
| 承認必須（レビュー） | **入れない** | 一人開発では自分のPRを自分で承認できず、**永久にマージできなくなる** |
| bypass actors | **空のまま** | 自分を入れると「困ったときは自分だけ無視できる」設定になり、仕組みで防ぐ意味が薄れる |

> **設定はGitの履歴に残らない。** GitHubのサーバー上にしか存在せず、PRも通らないため
> 「誰が・いつ・なぜ変えたか」が追えない。**だからここに書く**（Issue #87 / PR も記録として残す）。
