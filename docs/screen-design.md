...
| S-02 | タイムライン画面 | 投稿フォーム（本文＋画像選択）、投稿一覧（新着順）、いいね・コメント数、自分の投稿の編集・削除ボタン、ログアウト。Phase 3 で「全体／フォロー中」の切替タブを追加 | 1 |
...
**画像アップロードの流れ:** 「📷 画像を選択」でファイル選択ダイアログを開く →
クライアント側でJPEG/PNG・5MB以内かを検証（違反時は選択欄の下にエラーメッセージ）→
問題なければ選択画像のプレビューを投稿フォーム内に表示（「画像を削除」で選び直せる）→
